package leenjae.board.articleread.api;

import leenjae.board.articleread.service.response.ArticleReadPageResponse;
import leenjae.board.articleread.service.response.ArticleReadResponse;
...
public class ArticleReadApiTest {
    ...
    void readTest() {
        ArticleReadResponse response = articleReadRestClient.get()
                .uri("/v1/articles/{articleId}", 165710542932848690L)
                .retrieve()
    ...
    void readAllTest() {
        ...
        for(ArticleReadResponse article : response1.getArticles()) {
            System.out.println("article1.getArticleId() = " + article.getArticleId());
        }
        ...
        for(ArticleReadResponse article : response2.getArticles()) {
            System.out.println("article2.getArticleId() = " + article.getArticleId());
        }
    }
    ...
    void readAllInfiniteScrollTest() {
        List<ArticleReadResponse> response1 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L, 185290961257205760L))
                .retrieve()
